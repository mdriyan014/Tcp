...
def create_credentials_template():
    ...
    template = """# @TAITANIMUMCODEX Free Fire Bot Credentials
# Fill in your Free Fire account credentials below

# Format 1: Comma-separated (RECOMMENDED)
uid=4468970991,password=D_HJUVE_BY_BREXX_GAMING_MO0HM

# OR Format 2: Line-separated
# uid: 4468970991
# password: D_HJUVE_BY_BREXX_GAMING_MO0HM
...
def load_credentials_from_file(filename="NOOB_.txt"):
    """
    Load UID and password from NOOB_Apis.txt file
    """
    try:
        if not os.path.exists(filename):
...
🎯 Target: {target_uid}
🤖 Bot Name: @TAITANIMUMCODEX
✅ Status: Ready to join

📱 Player will see:
"@TAITANIMUMCODEX wants to join your team"
...
async def generate_guest_accounts(count=1, name="@TAITANIMUMCODEX", password_prefix="FF"):
...
async def generate_and_save_accounts(count, name="@TAITANIMUMCODEX", password_prefix="FF"):
    ...
    try:
        ...
        fields = {
            ...
            2: {
                ...
                6: "iG:[C][B][FF0000] @TAITANIMUMCODEX",  # Nickname
                7: 330,                   # Rank
                8: 1000,                  # Field 8
...
async def Room_Spam(Uid, Rm, Nm, K, V):
    fields = {
        ...
        2: {
            1: int(Rm),  
            2: "iG:[C][B][FF0000]@TAITANIMUMCODEX_Apis",  
            3: {
                2: 1,
                3: 1
            },
...
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TELEGRAM: [FF0000]
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
    fields = {
    1: 5,
    2: {
...
async def convert_kyro_to_your_system(target_uid, chat_id, key, iv, nickname="TAITANIMUMCODEX", title_id=None):
...
async def send_sticker(target_uid, chat_id, key, iv, nickname="TAITANIM"):
...
async def send_kyro_title_adapted(chat_id, key, iv, target_uid, nickname="TAITANIM"):
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="TAITANIMUMCODEX", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="@TAITANIMUMCODEX", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde TAITANIMUMCODEX ya meku agar tu noob bolra toh tu gay hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="@TAITANIMUMCODEX", title_id=title_id)
              ...
              # =================== EMOTE DETECTION ONLY ===================
                    ...
                    try:
                        ...
                        if str(invite_uid) in WHITELISTED_UIDS:
                            ...
                            try:
                                ...
                                bot_uid = 14010319252
                                message_text = f" Can't accept Your request Talk to TAITANIMUMCODEX"
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[B][C][FF0000]█▓▒░░ WELCOME TO TAITANIMUMCODEX BOT ░░▒▓█
[00FF00]»»————-　★　————-««
[FFFFFF]⚡ BOT POWERED BY:[FF0000]X³
[FFFFFF]🎯 STATUS: [00FF00]ACTIVE 24/7
[FFFFFF]📱 CONTACT: [FFFF00]@TAITANIMUMCODEX
[00FF00]»»————-　★　————-««
            ...
            while True:
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        always_allowed = {"11686472351", "14010319252"}
...
📝 Examples:
/bio Hello World!
/bio 🤖 Bot by TAITANIMUMCODEX
/bio Level 70 | Pro Player
/bio Add me: TAITANIMUMCODEX
...
"""
                                ...
                                credentials = load_credentials_from_file("TAITANIM_Apis.txt")
                                if not credentials:
...
[b][i][FFC0CB]youtube: NO USE[/b]

[b][c][FFC0CB]subcribe: my_channel[FFFFFF]
 
[b][i][FFA500]telegram: @TAITANIMUMCODEX[/b]

[b][c][FFA500]telegram contact: @TAITANIMUMCODEX[A52A2A]
 
Enjoy the bot my friend.......

[C][B][0000FF] Created by ROSHAN 
Modified by - BLACK BRO
"""
...
"""
                        ...
                        BLOCKED_NAMES = ["arbaz", "ARBAZ", "Arbaz", "TAITANIMUMCODEX"]  # Add your actual name
...
[9400D3]M A D E B Y [FF0000]B L A C K 6 6 6 F F
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TELEGRAM: [FF0000]@TAITANIMUMCODEX
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
...
async def MaiiiinE():
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 TAITANIMUMCODEX BOT - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 TAITANIMUMCODEX BOT - STARTING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 TAITANIMUMCODEX BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 TAITANIMUMCODEX BOT - ONLINE")
    print("=" * 50)
    print(f"🔹 UID: {TarGeT}")
