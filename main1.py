# ======================== IMPORTS =======================
import requests , os , psutil , jwt , pickle , json , binascii , time , urllib3 , base64 , datetime , re , socket , threading , ssl , pytz , aiohttp , traceback , signal , multiprocessing , asyncio , urllib.parse
...
def load_credentials_from_file(filename="HUNT_Apis.txt"):
    """
    Load UID and password from HUNT_Apis.txt file
    """
    try:
        if not os.path.exists(filename):
...
📱 Player will see:
"HUNT LEGACY wants to join your team"
    ...
    try:
        ...
        fields = {
            ...
            2: {
                ...
                6: "iG:[C][B][FF0000] developmode",  # Nickname
                7: 330,                   # Rank
                8: 1000,                  # Field 8
                ...
                27: {                    # Field 27 (critical for badge!)
                    1: 11,               # Field 27.1
                    2: 15027897210,      # Field 27.2 (your bot UID)
                    3: 9999              # Field 27.3
                },
                28: {},                   # Empty field 28
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]HUNT LG [00FF00]___X³____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @developmode\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
def get_player_ban_status(uid):
    try:
        url = f"https://banchecker-mu.vercel.app/bancheck?uid={uid}"
...
Region: {server_name}
[FFDD00]°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°
[00FF00][b][c]BOT MADE BY  @developmode
"""
            else:
...
def get_player_add(uid):
    try:
        url = f"https://danger-add-friend.vercel.app/adding_friend?uid=4270936858&password=developmode_94IWM_BY_SPIDEERIO_GAMING_JOX82&friend_uid={uid}"
        res = requests.get(url)
        data = res.json()
...
def get_player_basic(uid):
    try:
        url = f"https://info-api-delta.vercel.app/accinfo?uid={uid}&region=bd"
        ...
        level = basic.get("level", "None")
        like = basic.get("liked", "None")
        region = basic.get("region", "None")
        rank = basic.get("rank", "None")
        max_rank = basic.get("maxRank", "None")

        cs_rank = basic.get("csRank", "None")
        cs_max_rank = basic.get("csMaxRank", "None")
        version = basic.get("releaseVersion", "None")
        banner_id = basic.get("bannerId", "None")
        badge_count = basic.get("badgeCnt", "None")
        guild_name = clan.get("clanName", "None")

        return f"""
[C][B][FF9900]━━━━━━━━━━━━ PLAYER INFO ━━━━━━━━━━━━

[C][B][00FFFF]Name: [FFFFFF]{name}
[C][B][00FF00]Level: [FFFFFF]{level}
[C][B][FF00FF]Like: [FFFFFF]{like}
[C][B][00FFAA]Region: [FFFFFF]{region}

[C][B][FF4444]Rank: [FFFFFF]{rank}
[C][B][FF8800]BR Max Rank: [FFFFFF]{max_rank}
[C][B][AA00FF]CS Rank: [FFFFFF]{cs_rank}
[C][B][FF0088]CS Max Rank: [FFFFFF]{cs_max_rank}

[C][B][3399FF]Version: [FFFFFF]{version}
[C][B][FFFF00]Banner ID: [FFFFFF]{banner_id}
[C][B][66FF66]Badge Count: [FFFFFF]{badge_count}
[C][B][FF66CC]Guild: [FFFFFF]{guild_name}
...
def talk_with_ai(question):
    try:
        ...
        url = f"https://ai-asking.vercel.app/api/ask?key=dark&ask={q}"
...
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TELEGRAM: [FF0000]@developmode
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
    fields = {
    1: 5,
    2: {
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="HUNT GAMER", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="developmode", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde HUNT GAMER ya meku agar tu noob bolra toh tu g a y hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="developmode", title_id=title_id)
...
📝 Example: /spamroom 123456 15027897210 Hello World!
...
async def auto_reply_with_emote(emote_info, key, iv):
    ...
    try:
        ...
        bot_uid = 15027897210  # Replace with your bot's actual UID
              ...
              # =================== EMOTE DETECTION ONLY ===================
                ...
                if data_hex.startswith("0500") and insquad is None and joining_team == False:
                    try:
                        ...
                        if "13577606265" in WHITELISTED_UIDS:
                            ...
                            try:
                                ...
                                bot_uid = 15027897210
                                message_text = f" Can't accept Your request Talk to @developmode"
...
[FFFFFF]📱 CONTACT: [FFFF00]@developmode
[00FF00]»»————-　★　————-««
            ...
            while True:
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        # ============ BLOCK EVERYTHING FOR NON-WHITELISTED ============
                        uid_str = str(uid)
    
                        # List of always-allowed UIDs
                        always_allowed = {"13577606265", "15027897210"}
    
                        # Check if user is allowed
                        is_allowed = uid_str in always_allowed or uid_str in WHITELISTED_UIDS
    
                        if not is_allowed:
                            print(f"🛑 BLOCKING ALL COMMANDS FOR UID: {uid_str}")
        
                            # Send ONE block message
                            if not hasattr(TcPChaT, f'blocked_{uid_str}'):
                                block_msg = f"[B][C][FF0000]🚫 PRIVATE BOT!\n[FFFFFF]Contact @developmode"
                                await safe_send_message(XX, block_msg, uid, chat_id, key, iv)
                                setattr(TcPChaT, f'blocked_{uid_str}', True)
...
📝 Examples:
/bio Hello World!
/bio 🤖 Bot by HUNT GAMER
/bio Level 70 | Pro Player
/bio Add me: HUNT GAMER
...
"""
                                ...
                                credentials = load_credentials_from_file("HUNT_Apis.txt")
                                if not credentials:
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/wlremove'):
                            ...
                            # Check owner
                            if str(response.Data.uid) != "13577606265":
...
"""
                            ...
                            if str(response.Data.uid) != "13577606265":  # Replace with your actual UID
...
"""
                        ...
                        if inPuTMsG.strip() == '/wllist':
                            ...
                            # Check if owner
                            if str(response.Data.uid) != "13577606265":  # Your UID
...
👥 Whitelisted UIDs:"""
                            ...
                            for uid in WHITELISTED_UIDS:
                                if uid != "13577606265":  # Skip owner since already shown
                                    whitelist_msg += f"\n• {xMsGFixinG(uid)}"
                                    count += 1
                                    if count >= 20:
...
[b][i][FFC0CB]youtube: HUNT GAMER 99[/b]

[b][c][FFC0CB]subcribe: my_channel[FFFFFF]
 
[b][i][FFA500]telegram: @developmode[/b]

[b][c][FFA500]telegram contact: @developmode[A52A2A]
 
Enjoy the bot my friend.......

[C][B][0000FF] Created by developmode
Modified by - developmode
"""
...
"""
                        ...
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            test_msg = f"[B][C][FF0000]Hello {player_name}!"
...
[FF0000]ＷＥＬＣＯＭＥ [FFD700]{{player_name}}[FF0000] ＴＯ
[FF0000] M G 2 4  G A M E R   [FF0000]ＢＯＴ
[FF0000]━[FF0000]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━"""
...
[FF0000]/start       [FFFFFF]Start match
[FF0000]/exit        [FFFFFF]Leave squad
[FF0000]/3           [FFFFFF]Send 3-player invite
[FF0000]/5           [FFFFFF]Send 5-player invite
[FF0000]/6           [FFFFFF]Send 6-player invite
[FF0000]! (team code)   [FFFFFF]Join squad
[FF0000]/ghost (team code)  [FFFFFF]Ghost join squad
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
...
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FF0000]/e [emote]        [FFFFFF]Send emote to yourself
[FF0000]/e [uid] [emote]  [FFFFFF]Send emote to player
[FF0000]/e list           [FFFFFF]Show all emotes
[FF0000]/e list names     [FFFFFF]Show named emotes
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
...
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB][FF0000]@evos            [FFFFFF]Start evolution cycle
[FF0000]@sevos           [FFFFFF]Stop evolution cycle
[FF0000]/evo [num]       [FFFFFF]Send evo emote (1-18)
[FF0000]/evo [uid] [num] [FFFFFF]Send evo emote to player
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
"""
...
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FF0000]/fast [uid] [emote]    [FFFFFF]Fast emote spam
[FF0000]/p [uid] [emote] [num] [FFFFFF]Custom emote spam
[FF0000]/reject [uid]          [FFFFFF]Reject spam
[FF0000]/reject_stop           [FFFFFF]Stop reject spam
[FF0000]/msg [text] [times]    [FFFFFF]Message spam
[FF0000]/stop msg              [FFFFFF]Stop message spam
[FF0000]/mg [text] [repeats]   [FFFFFF]Wave message spam
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
...
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FF0000]/s1 [uid]  [FFFFFF]Spam with badge 1
[FF0000]/s2 [uid]  [FFFFFF]Spam with badge 2
[FF0000]/s3 [uid]  [FFFFFF]Spam with badge 3
[FF0000]/s4 [uid]  [FFFFFF]Spam with badge 4
[FF0000]/s5 [uid]  [FFFFFF]Spam with badge 50
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
...
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FF0000]/inv [uid]           [FFFFFF]Send group invite
[FF0000]/joinroom [id] [pass] [FFFFFF]Join custom room
[FF0000]/lag [team_code]     [FFFFFF]Lag attack squad
[FF0000]/stop lag            [FFFFFF]Stop lag attack
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
...
[FF0000]━[FF0000]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]─
[FF0000]/info [uid]       [FFFFFF]Player information
[FF0000]/status [uid]     [FFFFFF]Check player status
[FF0000]/gali [name]      [FFFFFF]Gali spam message
[FF0000]/likes [uid]      [FFFFFF]Send 100 likes
[FF0000]/ai [question]    [FFFFFF]Chat with AI
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
...
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FF0000]/help        [FFFFFF]Show this help menu
[FF0000]/admin       [FFFFFF]Admin/contact information
[FF0000]/quick [code] [emote] [uid] [FFFFFF]Quick emote attack
[FF0000]━[FF0000]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
...
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FF0000]/wllist          [FFFFFF]View whitelisted UIDs
[FF0000]/wladd (uid)     [FFFFFF]Add UID to whitelist
[FF0000]/wlremove (uid)  [FFFFFF]Remove UID from whitelist
[FF0000]/wlenable        [FFFFFF]Enable whitelist-only mode
[FF0000]/wldisable       [FFFFFF]Disable whitelist-only mode
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
...
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TELEGRAM: [FF0000]@developmode
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFFF00]💡   T I P: [FFFFFF]Use [FF0000]/e list names [FFFFFF]to see all named emotes!
[FFFF00]💡   T I P: [FFFFFF]Bot auto-joins squads & rejects requests automatically!
[FFFF00]💡   T I P: [FFFFFF]Use [FF0000]@evos [FFFFFF]for automatic evolution emote cycle!"""
...
async def MaiiiinE():
    ...
    print("📁 Loading credentials from HUNT_Apis.txt...")
    credentials = load_credentials_from_file("HUNT_Apis.txt")
    
    if not credentials:
        print("❌ Failed to load credentials!")
        print("💡 Please create HUNT_Apis.txt with your UID and password")
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 developmode BOT - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 developmode BOT - STARTING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 developmode BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
